...
```bash
chmod +x *.py *.sh
# 或直接使用：python3 scripts/init/init_project.py
```
...
```bash
# 添加别名到.bashrc或.zshrc
echo 'alias ai-copyright="python3 scripts/init/init_project.py"' >> ~/.bashrc
source ~/.bashrc
...
# 测试在任意目录创建项目
cd /tmp
python3 scripts/init/init_project.py test-anywhere
```
